...
r4a-cli --master http://10.42.0.1:3501 --secret <секрет> nodes list
R4A_MASTER=http://10.42.0.1:3501 r4a-tui   # дашборд; колонка "P2P" показывает прямые линки
```
...
curl -X POST http://10.42.0.1:3501/api/manifests \
  -H "Authorization: Bearer <секрет-или-токен>" \
...
- Web UI: `http://localhost:3502` — секрет `test_secret_for_cluster_123`
- API: `http://localhost:3501`, ingress: `http://localhost:3500`
- TUI: `R4A_MASTER=http://localhost:3501 r4a-tui` или `docker exec -it node-master r4a-tui`
...
| `R4A_ADMIN_SECRET` | Админ-доступ к API/Web UI |
